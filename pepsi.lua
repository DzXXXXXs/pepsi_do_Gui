...
function library:CreateWindow(options, ...)
	...
	if windowName and #windowName > 0 and library.WorkspaceName == "Shadow Hub Premium Scripts" then
		library.WorkspaceName = convertfilename(windowName, "Shadow Hub Premium Scripts")
	end
		...
		function tabFunctions:CreateSection(options, ...)
			...
			if isfolder and makefolder and listfiles and readfile and writefile then
				function sectionFunctions:AddPersistence(options, ...)
					...
					local realDropdownHolderList = Instance_new("UIListLayout")
					local dropdownEnabled = false
					if not isfolder("./Shadow Hub Premium Scripts") then
						makefolder("./Shadow Hub Premium Scripts")
					end
					local common_string = "./Shadow Hub Premium Scripts/" .. tostring(custom_workspace or library.WorkspaceName)
					local function resolvelist(nofold)
						if custom_workspace ~= options.Workspace then
							custom_workspace = options.Workspace
							common_string = "./Shadow Hub Premium Scripts/" .. tostring(custom_workspace or library.WorkspaceName)
						end
	...
	function windowFunctions:CreateDesigner(options, ...)
		...
		local daaata = {{"AddTextbox", "__Designer.Textbox.ImageAssetID", backgroundsection, {
			...
			Workspace = "Shadow Hub Premium Scripts Themes",
			Flags = flags,
			Suffix = "Theme",
			Desginer = true
